odoo.define('spa_management.website_spa_booking', function (require) {
    'use strict';
    ...
    $(document).on('click', "#submit_button", function () {
        ...
        var phone = $("#phone").val();
        var email = $("#email").val();
        var service = $('.check_box_spa:checkbox:checked');
        var chair = $("#chair").val();
        var list_service = [];
        ...
        if (name == "" || date == "" || time == "" || phone == "" || email == "" || list_service.length == 0) {
            ...
            if (isNaN(time_left_char) || isNaN(time_right_char) || time_separator != ":") {
                ...
                if ((time_left < 24) && (time_right < 60) && (time_left >= 0) && (time_right >= 0)) {
                    ...
                    $.ajax({
                        url: "/page/spa_details",
                        type: "POST",
                        dataType: "json",
                        data: booking_record,
                        success: function (data) {
                            window.location.href = "/page/spa_management/spa_booking_thank_you";
                        },
    ...
    $(document).on('click', "#check_button", function () {
        ...
        if (check_date != "") {
            ajax.jsonRpc("/page/spa_check_date", 'call', {
